...
func _init_connections() -> void:
	if not _inventory.is_connected("scene_changed", self, "_on_scene_changed"):
		assert(_inventory.connect("scene_changed", self, "_on_scene_changed") == OK)
	...
	if event is InputEventMouseButton:
		if event.pressed:
			if event.button_index == BUTTON_MIDDLE:
				...
				if _data.resource_exists(_inventory.scene):
					file_dialog.current_dir = _data.file_path(_inventory.scene)
					file_dialog.current_file = _data.filename(_inventory.scene)
...
func _check_path_ui() -> void:
	if _inventory.scene and not _data.resource_exists(_inventory.scene):
		set("custom_styles/normal", _path_ui_style_resource)
		hint_tooltip =  "Your resource path: \"" + _inventory.scene + "\" does not exists"
	else:
